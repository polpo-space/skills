...
- **Powered by jira CLI**: Fast, local access via the `jira` command
...
The skill uses the `jira` CLI to perform operations:
...
You need the `jira` CLI installed and configured.
...
### For Assignments
1. Use the correct user identifier (account ID, not display name)
...
- Assigns using display names (only account IDs work)
...
If the `jira` CLI is not installed:
